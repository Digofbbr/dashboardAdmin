    ...
    <div className="nav-item absolute right-1 top-16 bg-white dark:bg-[#42464D] p-8 rounded-lg w-96">
      ...
      <div className="mt-5 ">
        <div className='flex gap-5 items-center border-color border-b-1 pb-6'>
          <img className="rounded-full h-20 w-20" src={avatar} alt="avatar"/>
          <div>
            <p className='font-semibold text-xl dark:text-gray-200'>Michael Roberts</p>
            <p className='text-gray-500 text-sm dark:text-gray-400'>Administrator</p>
            <p className='text-gray-500 font-semibold text-sm dark:text-gray-400'>info@shop.com</p>
          </div>

        </div>
        {userProfileData?.map((item, index) => (
          <div key={index} className="flex gap-5 border-b-1 border-color p-4 hover:bg-light-gray cursor-pointer  dark:hover:bg-[#42464D]">
            <div style={{backgroundColor: item.iconBg, color: item.iconColor}} className='text-xl rounded-lg p-3 hover:bg-light-gray'>
              {item.icon} 
            </div>
            <div>
